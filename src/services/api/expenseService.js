import expensesData from "@/services/mockData/expenses.json";

class ExpenseService {
  constructor() {
    this.storageKey = "farmtrack_expenses";
    this.initializeData();
  }

  initializeData() {
    const stored = localStorage.getItem(this.storageKey);
    if (!stored) {
      localStorage.setItem(this.storageKey, JSON.stringify(expensesData));
    }
  }

  getData() {
    const stored = localStorage.getItem(this.storageKey);
    return stored ? JSON.parse(stored) : [];
  }

  saveData(data) {
    localStorage.setItem(this.storageKey, JSON.stringify(data));
  }

  async getAll() {
    await this.delay(300);
    return [...this.getData()];
  }

  async getById(id) {
    await this.delay(200);
    const data = this.getData();
    const expense = data.find(item => item.Id === parseInt(id));
    return expense ? { ...expense } : null;
  }

  async getByFarmId(farmId) {
    await this.delay(200);
    const data = this.getData();
    return data.filter(expense => expense.farmId === farmId).map(expense => ({ ...expense }));
  }

  async create(expenseData) {
    await this.delay(400);
    const data = this.getData();
    const newId = Math.max(...data.map(item => item.Id), 0) + 1;
    
    const newExpense = {
      Id: newId,
      ...expenseData,
      createdAt: new Date().toISOString()
    };
    
    data.push(newExpense);
    this.saveData(data);
    return { ...newExpense };
  }

  async update(id, expenseData) {
    await this.delay(400);
    const data = this.getData();
    const index = data.findIndex(item => item.Id === parseInt(id));
    
    if (index === -1) {
      throw new Error("Expense not found");
    }

    const updatedExpense = {
      ...data[index],
      ...expenseData,
      Id: parseInt(id)
    };
    
    data[index] = updatedExpense;
    this.saveData(data);
    return { ...updatedExpense };
  }

  async delete(id) {
    await this.delay(300);
    const data = this.getData();
    const filteredData = data.filter(item => item.Id !== parseInt(id));
    
    if (filteredData.length === data.length) {
      throw new Error("Expense not found");
    }
    
    this.saveData(filteredData);
    return true;
  }

  delay(ms) {
    return new Promise(resolve => setTimeout(resolve, ms));
  }
}

export default new ExpenseService();